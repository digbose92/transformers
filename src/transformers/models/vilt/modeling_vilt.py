...
class ViltEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        #initialize the bottleneck embedding layer
        self.bottleneck=nn.Embedding(config.num_bottleneck_tokens,config.hidden_size)
    ...
    def forward(
        ...
        #bottleneck embeddings extract from bottleneck layer and expand 
        #to the same size as text_embeds and image_embeds
        bottleneck_masks=torch.ones(image_embeds.shape[0],self.config.num_bottleneck_tokens)
        bottleneck_embeds=self.bottleneck_embeddings(torch.arange(self.config.num_bottleneck_tokens).unsqueeze(0).repeat(image_embeds.shape[0],1))
        
        
        # PART 4: concatenate the embeddings
        bn_image_embeddings = torch.cat([bottleneck_embeds, image_embeds], dim=1)
        bn_text_embeddings = torch.cat([text_embeds,bottleneck_embeds], dim=1)


        #masks of bottleneck and image and text combinations
        bn_image_masks = torch.cat([bottleneck_masks, image_masks], dim=1)
        bn_text_masks= torch.cat([attention_mask,bottleneck_masks], dim=1)

        #return all the combinations of embeddings and masks 
        return text_embeds, image_embeds, bn_image_embeddings, bn_text_embeddings, bn_image_masks, bn_text_masks
...
### ViLTModel (not initializing with pretreined weights)
class ViltBNModel(nn.Module):
    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config
    ...
    def forward(
        ...
        text_embeds, image_embeds, bn_image_embeddings, bn_text_embeddings, bn_image_masks, bn_text_masks = self.embeddings(
            input_ids,
            attention_mask,
            token_type_ids,
            pixel_values,
        ...
        # We can provide a self-attention mask of dimensions [batch_size, from_seq_length, to_seq_length]
        # ourselves in which case we just need to make it broadcastable to all heads.
        extended_bn_image_attention_mask: torch.Tensor = self.get_extended_attention_mask(bn_image_masks, input_shape)
        extended_bn_text_attention_mask: torch.Tensor = self.get_extended_attention_mask(bn_text_masks, input_shape)
        #first forward pass (bn + imkage combination)

        bn_image_encoder_outputs = self.encoder(
            bn_image_embeddings,
            attention_mask=extended_bn_image_attention_mask,
            head_mask=head_mask,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        ...
        #second forward pass (bn + text combination)
        bn_text_encoder_outputs = self.encoder(
            bn_text_embeddings,
            attention_mask=extended_bn_text_attention_mask,
            head_mask=head_mask,
            output_attentions=output_attentions,
            output_hidden_states=output_hidden_states,
            return_dict=return_dict,
        ...
        #extract the bottleneck segments fromm bn_text_encoder_outputs by using specific number of tokens 
        text_feats, bn_feats = (
            bn_text_encoder_outputs.last_hidden_state[:, :text_embeds.shape[1],:],
            bn_text_encoder_outputs.last_hidden_state[:, text_embeds.shape[1]:,:],
        )

        #average pool the bn outputs 
        pooled_output = bn_feats.mean(dim=1)
        sequence_output = self.layernorm(bn_feats)
        #pooled_output = self.pooler(bn_feats) if self.pooler is not None else None
